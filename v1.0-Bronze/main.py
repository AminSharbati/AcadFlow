# AcadFlow v1.0 - Bronze Edition
# سیستم جامع مدیریت آموزشی | نسخه برنزی
# Code First with SQLAlchemy + Tkinter
# ارائه شده توسط تیم داتین آریا | ۱۴۰۵

import tkinter as tk
from tkinter import ttk, messagebox
from sqlalchemy import create_engine, Column, Integer, String, ForeignKey, Float, event
...
class Student(Base):
    ...
    COLUMNS = {
        ...
        "کد ملی": "national_code", "ترم ورود": "entrance_term",
        "مقطع": "graduation", "موبایل": "mobile", "ایمیل": "email", "رشته": "major"
    }
...
class Master(Base):
    ...
    COLUMNS = {
        ...
        "مدرک": "degree", "موبایل": "mobile", "ایمیل": "email"
    }
...
class Lesson(Base):
    ...
    COLUMNS = {
        "شناسه": "id", "نام": "name", "واحد": "unit", "رشته": "major"
    }
...
# برنامه اصلی
# ============================================================
class AcadFlowApp_V1:
    def __init__(self, root):
        self.root = root
        self.root.title(f"{APP_NAME} - سیستم مدیریت آموزشی | نسخه برنزی {VERSION}")
    ...
    def _setup_styles(self):
        ...
        style.configure('Treeview', font=self.font_normal, rowheight=27)
    ...
    def _create_header(self):
        ...
        tk.Label(header, text=f"نسخه {VERSION} | نسخه برنزی | {COMPANY}  ",
    ...
    def _create_notebook(self):
        ...
        tab_configs = [
            ('student', Student, "👨‍🎓 دانشجویان", [
                ("نام", "name"), ("کد دانشجویی", "student_code"),
                ("کد ملی", "national_code"), ("ترم", "entrance_term", "optional"),
                ("مقطع", "graduation"), ("موبایل", "mobile"),
                ("ایمیل", "email", "optional"), ("رشته", "major", "combo", self.majors_list)
            ]),
            ('master', Master, "👨‍🏫 اساتید", [
                ("نام", "name"), ("کد ملی", "national_code"),
                ("مدرک", "degree"), ("موبایل", "mobile"),
                ("ایمیل", "email", "optional")
            ]),
            ('lesson', Lesson, "📚 دروس", [
                ("نام درس", "name"), ("تعداد واحد", "unit", "int"),
                ("رشته", "major", "combo", self.majors_list)
            ]),
            ('presentation', Presentation, "📅 ارائه دروس", [
                ("استاد", "master_id", "combo_fk", Master, 'id', 'name'),
                ("درس", "lesson_id", "combo_fk", Lesson, 'id', 'name'),
                ("روز", "day_hold", "combo", self.week_days),
                ("شروع", "start_time", "int_optional"),
                ("پایان", "finish_time", "int_optional")
            ]),
            ('selection', Selection, "📝 انتخاب واحد", [
                ...
                ("درس", "presentation_id", "combo_fk_filtered", Presentation, 'id', 'display_name'),
                ("سال", "year_education", "int_optional"),
    ...
    def _build_tab(self, tab_key):
        ...
        ctx_menu.add_command(label="🔁تازه‌سازی", command=lambda: self._refresh_tab(tab_key))
        ctx_menu.add_command(label="❌حذف", command=lambda: self._delete_record(tab_key))
    ...
    def _get_form_data(self, tab_key):
        ...
        for f in info['fields']:
            ...
            if field_type.startswith('combo_fk'):
                fk_model = f[3]
                name_field = f[5]
                for r in self.session.query(fk_model).all():
        ...
        for i, rec in enumerate(records):
            ...
            for attr in col_keys:
                ...
                if info['model'] == Presentation:
                    ...
                    elif attr == 'presentation_id':
                        if rec.presentation:
                            value = f"{rec.presentation.lesson.name} ({rec.presentation.master.name})" if rec.presentation.lesson and rec.presentation.master else 'نامشخص'
                        else:
                            value = 'نامشخص'
    ...
    def _on_tab_change(self, event):
        ...
        if idx < len(keys):
            self._refresh_tab(keys[idx])
            if keys[idx] == 'selection':
                info = self.tabs['selection']
                info['entries']['major_filter'].set('')
                info['entries']['student_id'].set('')
                info['entries']['presentation_id']['values'] = []
